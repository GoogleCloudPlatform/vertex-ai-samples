...
def save_model_artifact(destination: str) -> None:
  ...
  model = timm.create_model(predictor.TimmPredictor.TIMM_MODEL_NAME,
                            pretrained=True)
  dest_file = os.path.join(destination, 
                           predictor.TimmPredictor.WEIGHTS_FILE)
...
def upload_model(config: CPRConfig) -> aiplatform.Model:
  ...
  ar_tag = (f"{config.region}-docker.pkg.dev/{config.project_id}"
