...
def parse_args():
  ...
  parser = argparse.ArgumentParser(
      description='TF-Keras Text Classification with GPU on Stack Overflow Questions')
...
def main():
  ...
  print(f'Num GPUs Available: {len(devices_gpu)}')

  strategy = None
  if len(devices_gpu) > 1:
  ...
  raw_train_ds, raw_val_ds, raw_test_ds = load_dataset(dataset_dir, args.batch_size)
